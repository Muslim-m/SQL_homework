...
select  distinct saleid , customername, price, saledate from sales as s1
where exists
(select saleid , customername, price, saledate from sales as s2  where s1.customername=s2.customername and saledate >= '2024-03-01'
...
select product  , sum(quantity*price) from sales
group by product
having sum(quantity*price)=(
select max(total) from
(select sum(quantity*price) as total  from sales
...
WITH RoutePaths AS (
    ...
    -- Recursive step: keep extending the path
    SELECT 
    ...
    WHERE RP.Route NOT LIKE '%' + R.ArrivalCity + '%'  -- avoid loops
)

-- Select only complete routes ending in Khorezm
...
ORDER BY 
    Cost ASC -- gives both cheapest and most expensive due to WITH TIES + ROW_NUMBER
...
SELECT c.name, c.category, c.price
FROM cte c
INNER JOIN cte2
  ON c.category = cte2.category
WHERE c.price > cte2.avg_price;
--16.Find the products that have a stock count lower than the highest stock count.

select category, max(stock) from products
group by category;
SELECT *
FROM Products
WHERE Stock < (
